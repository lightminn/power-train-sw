...
Rocker x Bogie 전체 파라미터 최적 탐색 — JAX GPU 가속 버전

가속 전략:
  1. kin_sim: JAX vmap + 배치 Newton 솔버 (GPU 병렬 160포인트 동시)
  2. calc_dynamics: for 루프 → numpy 벡터 연산
  3. calc_stability: 법선력/TOI 벡터화
  4. 최적화: scipy differential_evolution (population 내 평가 병렬화 가능)

예상 속도 향상:
  - RTX 3090: ~10-30x (JIT 컴파일 후)
  - A100:     ~20-50x
"""
import os
import sys
...
print('=== ZETIN GPU 가속 최적화 v3 (JAX + Newton batch) ===')
...
# ═══════════════════════════════════════
# JIT 워밍업 (첫 호출 시 컴파일 → 이후 초고속)
# ═══════════════════════════════════════
print('JAX JIT 워밍업 중 (첫 컴파일, 1-2분 소요)...')
_t_warmup = time.time()
_warmup_p = dict(p0)
_warmup_p['rocker_mode'] = 'frame'
_warmup_p['bogie_mode'] = 'frame'
_warmup_p['T_r'] = 0.3; _warmup_p['S_r1'] = 0.2; _warmup_p['S_r2'] = 0.15
_warmup_p['th_r1'] = 0.3; _warmup_p['th_r2'] = 0.3; _warmup_p['j_r'] = 0.5
_warmup_p['T_b'] = 0.2; _warmup_p['S_b1'] = 0.18; _warmup_p['S_b2'] = 0.12
_warmup_p['th_b1'] = 0.3; _warmup_p['th_b2'] = 0.3; _warmup_p['j_b'] = 0.5
_warmup_p['c_b'] = 0.14; _warmup_p['d_b'] = 0.14

_wp_arr = pack_params_auto(_warmup_p)
_flat_x, _flat_y = gen_terrain('flat', _warmup_p)
_flat_env = calc_envelope_gpu(_flat_x, _flat_y, _warmup_p['R_w'])

# 실제 N_PTS 크기로 워밍업 (JIT 캐시 적중 보장)
_warmup_xa = np.linspace(_flat_x[0] + 0.3, _flat_x[-1] - 0.3, N_PTS)
_R_warmup = kin_sim_gpu(_warmup_xa, _flat_x, _flat_env, _wp_arr, _warmup_p)
print(f'JAX 워밍업 완료! ({time.time()-_t_warmup:.1f}초, 실패율: {_R_warmup["fail_rate"]*100:.1f}%)\n')


# ═══════════════════════════════════════
# [SECTION 4] 목적함수 (GPU 가속)
# ═══════════════════════════════════════
...
# [SECTION 3] 탐색 공간
# ═══════════════════════════════════════
lb = [2, 2, 0.20, 0.15, 60, 0, 0, 0.30, 0.15, 0.15, 60, 0, 0, 0.30]
ub = [2, 2, 0.45, 0.35, 160, 35, 35, 0.70, 0.35, 0.25, 160, 40, 40, 0.70]
bounds = list(zip(lb, ub))

# ═══════════════════════════════════════
# [SECTION 7] 최적화 실행
# ═══════════════════════════════════════
print('differential_evolution 실행 중 (GPU 가속, maxiter=200)...\n')
tic = time.time()
