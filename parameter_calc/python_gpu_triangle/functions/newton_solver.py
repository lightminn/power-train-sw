...
def kin_sim_gpu(x_arr, x_t, y_t_env, p_arr, p_dict):
    ...
    # brk_v: 브래킷 피벗 → 휠 축 수직 오프셋. p_arr 길이 21 미만이면 0으로 처리(하위호환).
    brk_v = np.float32(float(p_arr[20])) if len(p_arr) > 20 else np.float32(0.0)
    ...
    # brk_v 적용 시 본체가 그만큼 위로 떠야 하므로 초기 추정에 가산.
    y_init = np.interp(x_arr, np.array(x_t), np.array(y_t_env)).astype(np.float32) + R_w + brk_v
