...
def calc_stability_gpu(R, x_arr, x_t, y_t_raw, y_t_env, p):
    """전복 안정성 분석 — 벡터화 버전 (Phase 1: 동적 CG 보정 포함)"""
    ...
    N = len(x_arr)
    # 주의: 여기서 W는 전체-로봇 스케일이지만, 이 모듈의 법선력(Nr/Nm/Nf_raw)은
    # liftoff 부호 판정과 ZMP/TOI의 가중비율에만 쓰여 스케일 불변이다(절반/전체 결과 동일).
    # 따라서 calc_dynamics의 면-기준(W_side) 정정과 무관하게 그대로 둔다.
    W = p['mass'] * p['g']

    # ── 종방향 가속도로 인한 CG 유효 변위 ──
    # 사이비-힘 m*a_long이 h_CG 높이에 작용 → 접촉면 기준 CG_x가
    # (a_long/g)·h_CG 만큼 이동한 것과 등가. 가속 시 후방으로(반대 부호) 기울임.
    a_long = R.get('a_long', np.zeros(N))
    dx_inertia = -(a_long / g) * h_CG  # 가속 시 후방 이동 (전방 가속 → CG가 뒤로 보이는 효과)

    x_cg_static = R.get('xcg', x_arr + CG_offset * np.cos(R['ar']))
    x_cg = x_cg_static + dx_inertia
