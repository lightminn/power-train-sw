...
지원 타입:
    ...
    'curved_ramp'  대칭 cosine 둔덕 (폭 1.5m, 높이 p['obs_h']=0.15m 기본)
    'incline_15'   15° 일정 경사 (진입 평지 1.5m → 경사 → 정점 평지)
    'incline_30'   30° 일정 경사 (가파른 경사)
"""
import numpy as np
...
def gen_terrain(terrain_type, p):
    ...
    elif t in ('incline_15', 'incline_30'):
        # 일정 경사 슬로프: [0, 1.5m] 평지 → [1.5, 1.5+L_slope] 일정 경사 → 정점 평지.
        # L_slope = 2.0m 으로 두어 경사 후 안정 구간 확보.
        angle_deg = 15.0 if t == 'incline_15' else 30.0
        angle_rad = np.deg2rad(angle_deg)
        x_start = 1.5
        L_slope = 2.0
        x_end_slope = x_start + L_slope * np.cos(angle_rad)
        peak_h = L_slope * np.sin(angle_rad)

        # 평지 → 경사 → 정점 평지
        in_slope = (x_t >= x_start) & (x_t < x_end_slope)
        in_top = x_t >= x_end_slope
        y_t[in_slope] = (x_t[in_slope] - x_start) * np.tan(angle_rad)
        y_t[in_top] = peak_h

    elif t == 'curved_ramp':
        # Raised-cosine bump: y = h/2 * (1 + cos(2π·u)) for |u| ≤ 0.5, else 0.
        # 정점 곡률반경 R = W²/(2π²·h) ≈ 0.38m (h=0.15, W=1.5) — 휠 반경 100mm 대비 적당한 챌린지.
        bump_h = p.get('obs_h', 0.15)
        bump_w = 1.5
        x_c = 3.0
        u = (x_t - x_c) / bump_w
        in_bump = np.abs(u) <= 0.5
        y_t[in_bump] = bump_h * 0.5 * (1.0 + np.cos(2.0 * np.pi * u[in_bump]))
