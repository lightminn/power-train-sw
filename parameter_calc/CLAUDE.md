...
> ⛔ **트랙 종료 — 2026-07-19.** 최종 CAD 가 확정돼 이 트랙은 더 이상 손대지 않는다.
> 버그를 발견해도 **보고만** 하고 `f_opt 0.2004` 를 재계산하지 않는다.
> 계산 질량은 **50 kg 확정**이며 86 kg 재최적화는 예정에 없다.
> 아래 내용은 종료 시점의 기록이다.
...
- 스펙: R_w 100 mm, TAU_REF 15 Nm, W_bot 400~700 mm. (질량 가정은 50 kg로 확정 — 구 86 kg 설계추정치로 재최적화하지 않음)
