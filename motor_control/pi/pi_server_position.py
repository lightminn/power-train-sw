...
BRAKE_RESISTANCE = 2.0   # Ω

LOG_DIR  = os.path.dirname(os.path.abspath(__file__))
LOG_FILE = os.path.join(LOG_DIR, "robot_server_pos.log")
# ──────────────────────────────────────────────────

def setup_logger():
    logger = logging.getLogger("robot_pos")
...
def apply_dc_protection(drv):
    try:
        ...
        drv.config.dc_max_negative_current        = -3.0
        drv.config.dc_bus_overvoltage_trip_level  = 16.0
...
def apply_control_gains(ax):
    ...
    ax.encoder.config.bandwidth              = 15
    ax.controller.config.pos_gain            = 20.0
    ax.controller.config.vel_gain            = 0.10
...
def enter_closed_loop(drv, ax):
    ...
    log.info(f"ODrive 준비 완료  "
             ...
             f"pos_gain={ax.controller.config.pos_gain}  "
...
def control_loop(drv_box, ax_box):
    dt        = 1.0 / CONTROL_HZ
    log_every = max(1, round(CONTROL_HZ / LOG_HZ))
    tick      = 0
    pos_sp    = None  # 첫 tick에 현재 위치로 초기화
    ...
    while _running:
        ...
        try:
            ...
            # ── 속도 적분 → 위치 setpoint ──
            pos_sp += vel * dt
            ax.controller.input_pos = pos_sp

            # ── 상태 로그 ──
            if tick % log_every == 0:
                pos_est = ax.encoder.pos_estimate
                vel_est = ax.encoder.vel_estimate
                iq      = ax.motor.current_control.Iq_measured
                vbus    = drv.vbus_voltage
                log.debug(f"[state] vel={vel:+.3f}  pos_sp={pos_sp:+.4f}  "
                          f"pos_est={pos_est:+.4f}  err={pos_sp-pos_est:+.4f}  "
                          f"vel_est={vel_est:+.3f}  Iq={iq:+.3f}A  "
...
def main():
    ...
    log.info(f"=== robot_server_pos 시작  로그파일: {LOG_FILE} ===")
    log.info(f"설정: MAX_VEL={MAX_VEL}  CONTROL_HZ={CONTROL_HZ}  모드=포지션제어")
