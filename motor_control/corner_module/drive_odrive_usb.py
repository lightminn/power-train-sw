...
Enum 경로 근거
--------------
`odrive_dualsense_vel_test.py` 는 클래스 기반 API 를 사용한다:
    ControlMode.VELOCITY_CONTROL   (= ControlMode enum)
    InputMode.PASSTHROUGH          (= InputMode enum)
    AxisState.CLOSED_LOOP_CONTROL  (= AxisState enum)
    AxisState.IDLE                 (= AxisState enum)

`init_odrive.py` 는 `from odrive.enums import *` 로 플랫 이름
(AXIS_STATE_IDLE 등)을 사용하지만 velocity-control 참조는 없으므로,
속도 제어 소스-오브-트루스인 dualsense 스크립트의 클래스 기반 API 를
따른다.

텔레메트리 경로 (두 스크립트 모두 axis1 직접 접근):
