...
▸ 저속 HALL 코깅존 대응: 플로어(`--min-rev`)는 폐지(기본 0, 스펙 r6 §2.2 D3) —
  저속 보상은 `--friction-ff`(torque feedforward, §2.2b)를 쓴다. 플로어 재도입은
  커미셔닝 재량(옵션은 보존, 0=off). 조향은 링키지 범위 내.

실행 (컨테이너 안, motor_control/ 에서 · can0 UP · DualSense 연결):
  python3 -m chassis.teleop_dualsense --diagnostic-direct-can
  비대화형 확인: 위 명령에 --confirm-arm-stowed 추가
  옵션: --v-max 1.5  --omega-max 1.2  --min-rev 0  --channel can0
"""
import sys
import time


DIRECT_CAN_DIAGNOSTIC_NOTICE = (
...
def require_diagnostic_direct_can(parser, args, input_fn=None):
    """Fail before hardware initialization unless diagnostic use is explicit."""
    if not args.diagnostic_direct_can:
        parser.error(DIRECT_CAN_DIAGNOSTIC_NOTICE)
    if args.confirm_arm_stowed:
        return
    if input_fn is None:
        input_fn = input
    try:
        confirmed = input_fn(ARM_STOWED_PROMPT)
    except EOFError:
        confirmed = ""
    if confirmed.strip() != "yes":
        parser.error("arm-stowed confirmation required: enter exact string yes")
