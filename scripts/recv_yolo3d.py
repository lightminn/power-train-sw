...
_PACKET_FIELDS = {"session_id", "frame", "t", "w", "h", "dets"}
_DETECTION_FIELDS = {"cls", "conf", "box", "xyz", "d", "az", "el"}
...
def _valid_detection(value, width, height):
    if not isinstance(value, dict) or set(value) != _DETECTION_FIELDS:
        return False
    if not isinstance(value["cls"], str) or not value["cls"]:
        return False
    if not _finite(value["conf"]) or not 0.0 <= value["conf"] <= 1.0:
        return False
    box = value["box"]
    if not isinstance(box, list) or len(box) != 4 or not all(map(_finite, box)):
        return False
    x1, y1, x2, y2 = box
    ...
    xyz = value["xyz"]
    if xyz is None:
        return value["d"] is None and value["az"] is None and value["el"] is None
    if not isinstance(xyz, list) or len(xyz) != 3 or not all(map(_finite, xyz)):
        return False
    return (
        _finite(value["d"])
        and value["d"] >= 0
        and _finite(value["az"])
        and _finite(value["el"])
    )


def parse_coord_packet(data):
    """Return one exact-schema packet, or None for an untrusted datagram."""
    if not isinstance(data, bytes) or not data or len(data) > 65535:
        return None
    try:
    ...
    if not isinstance(packet["session_id"], str) or not packet["session_id"]:
        return None
    if not _integer(packet["frame"]):
        return None
    if not _finite(packet["t"]):
        return None
    if not _integer(packet["w"], minimum=1) or not _integer(packet["h"], minimum=1):
        return None
    detections = packet["dets"]
    if not isinstance(detections, list):
        return None
    if not all(
        _valid_detection(detection, packet["w"], packet["h"])
...
class CoordPacketTracker:
    """Latest-only session/sequence gate for validated coordinate packets."""
    ...
    def update(self, packet):
        ...
        if previous is not None and session_id == previous["session_id"]:
            if packet["frame"] <= previous["frame"]:
                return False
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--width", type=int, default=1280,
                   help="송신 L515 color 영상 가로 해상도 (기본 1280)")
    p.add_argument("--height", type=int, default=720,
                   help="송신 L515 color 영상 세로 해상도 (기본 720)")
...
class CoordReceiver(threading.Thread):
    ...
    def __init__(self, port: int):
        ...
        self._rx_t = 0.0
        self._tracker = CoordPacketTracker()
        self.start()
    ...
    def run(self) -> None:
        while True:
            try:
                ...
                continue
            pkt = parse_coord_packet(data)
            if pkt is None or not self._tracker.update(pkt):
                continue
            with self._lock:
