...
- **CAN 간섭 결함 수정(2026-09-09)** — CAN/USB 단일 소유권, 소유자 정지 래치 후
  ...
  배포 후 **13·14가 재소실**해 제어/워치독 정지 후에도 8/10이었다. 후속 USB 진단에서
  MKS 보드의 node 13/14·500 kbps 설정은 정상이고 실제 firmware는 0.5.1 unreleased였다.
  **보드 CAN부만 동일 baud로 재초기화해 10/10을 복구**했다. 13/14 제한 조회 60초에서
  지속 소실은 재현되지 않았지만 일부 응답 누락은 있었다. MKS 송신 경합은 소스 반례가
  확인된 유력 후보였다. 이후 패치·실측 범위는 다음 MKS 항목을 따른다. 과거 장애 원인
  확정과 영구 안정성·주행·NVM 인수는 미완료다.
  `docs/reports/2026-09-09-mks-usb-can-diagnosis.md`가 당시 진단 기록이다.
...
- 캘리 NVM: 운용 정본에는 node 11/12의 전원사이클 3회 직진입과 나머지 4축의 전원
  재인가 후 재캘리 없는 운용이 기록돼 있다. **13/14는 9/9 USB 플래그 직접 대조와
  수정 firmware 적용 후 전원 재인가1회를 확인**했다. 13~16의 동일한3회 반복시험과
  15/16의 USB 플래그 직접 대조는 아직 미완료다.
