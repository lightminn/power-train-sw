...
- **CAN 간섭 결함 수정(2026-09-09)** — CAN/USB 단일 소유권, 소유자 정지 래치 후
  ...
  **보드 CAN부만 동일 baud로 재초기화해 10/10을 복구**했다. 13/14 제한 조회 60초에서
  ...
  확인된 유력 후보이며 실물 HAL값·원인 확정·영구 수정·주행·NVM 인수는 미완료다.
  `docs/reports/2026-09-09-mks-usb-can-diagnosis.md`를 최신 실측으로 따른다.
...
- 실제 firmware는 node 13/14 보드 USB에서 **0.5.1, unreleased=1**로 확인했다(2026-09-09).
