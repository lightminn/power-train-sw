...
WP5.2 팔 gate와 계약 출력의 launch 조합은 다음처럼 고정한다.

| `contract_v2_verified` | `arm_gate_mode` | 용도와 출력 계약 |
|---|---|---|
| `false` (기본) | `production` (기본) | `/arm_status`가 fresh locked가 아니면 motion hold. `/chassis_mode`는 `CORNERING`만 10 Hz로 발행하고 `DRIVING`, `MISSION_STOP`, `ArrivalStatus`는 발행하지 않는다. |
| `false` | `arm_absent_field` | 팔을 물리적으로 제거한 파워트레인 단독 실차 HIL 전용. publisher가 0일 때만 내부 fresh `STOWED_LOCKED` mock으로 팔 freshness를 대체하며 계약 출력은 계속 compatibility lock이다. |
| `true` | `production` | 양 팀 contract-v2 검증 뒤에만 사용. Task 5 supervisor 연결 전 기본 mode intent는 `STOW_REQUEST`다. |
| `true` | `arm_absent_field` | 생산 승인 조합이 아니다. 코드가 `MISSION_STOP`과 `ArrivalStatus`를 계속 차단한다. |

기본 launch는 안전하게 `false + production`이므로 팔 publisher가 없으면 주행하지 않는다. 기존
WP5.1 파워트레인 단독 HIL에서만 다음처럼 opt-in한다.

```bash
ros2 launch powertrain_ros wp5_control.launch.py \
  stop_mm:=<provisional-mm> arm_gate_mode:=arm_absent_field
```

이 명령 전 운영자는 바퀴 6개 부양과 팔의 기계적 접힘·고정을 육안 확인하고 확인자를
journal/운영 로그에 기록해야 한다. 노드는 활성 profile과 절차를 warn으로 남긴다.
`/arm_status` publisher가 나타나면 다음 50 Hz tick부터 mock을 버리고 실제 heartbeat 기준
default-deny로 복귀한다. 이 profile은 US-100, motor fault/stale, E-stop, command watchdog,
authority gate를 우회하지 않으며 대회 production launch에 사용하지 않는다.
...
| `chassis_node` | `contract_v2_verified` | `false` | `false`는 `CORNERING`만 발행하는 compatibility lock; `true`의 기본 intent는 `STOW_REQUEST` |
| `chassis_node` | `arm_gate_mode` | `production` | `arm_absent_field`는 publisher 0 + 운영자 육안확인·기록을 전제로 한 단독 실차 HIL 전용 |
...
| 서비스 | `/chassis_node/arm_lock_override` | `std_srvs/SetBool` | contract-v2 remote owner + stale arm + no latch/violation + `IDLE`/arm hold에서만 요청 |

`arm_lock_override=true`는 팔 freshness만 `REMOTE_ARM_OVERRIDE` 저속 profile로 대체한다.
활성화 시 mode intent를 `STOW_REQUEST`로 고정하고 이전 주행 명령을 폐기한다. `GRIP_LOST` latch,
...
`src/powertrain_ros/powertrain_ros/contract.py`가 문자열 어휘의 단일 출처다. 팔 contract-v2는
`DONE`을 발행하지 않으며 파워트레인도 이를 주행 재개 권위로 쓰지 않는다. 작업 완료 권위는
