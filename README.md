...
│   │   ├── realsense_stream.py         # RealSense color+depth UDP H.264 송신
...
| `vision/realsense_stream.py` | RealSense color+depth → GStreamer H.264 UDP 송신. `--mode sidebyside`(color\|depth, 30fps) / `overlay`(depth를 color 위에 반투명). 수신은 `scripts/recv_stream.sh` |
...
RealSense D435i (RGB-D) 점검·스트리밍:

```bash
# (Jetson 컨테이너 안) depth+color 점검
python3 motor_control/vision/realsense_test.py

# (Jetson 컨테이너 안) color+depth 스트리밍 (sidebyside: color|depth / overlay)
python3 motor_control/vision/realsense_stream.py --host <노트북IP> [--mode overlay]
