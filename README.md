...
> **테스트 axis는 axis1로 통일.** 모든 단축 테스트와 운영 스크립트는
> `axis1`을 대상으로 한다. (`odrive_calibration.py`의 boot-safety 설정만
> 양쪽 axis 모두 끄는 방어 코드로 axis0/axis1 함께 다룸 — 테스트 대상이
> 아니므로 예외)
...
| 파일 | 무엇을 하는가 | 비고 |
| --- | --- | --- |
| `odrive_calibration.py` | `axis1`에 `EncoderMode.HALL` · `pole_pairs=5` · `cpr=30` · `MotorType.HIGH_CURRENT` 강제 설정 후 풀 캘리 (모터 → 홀 폴라리티 → 오프셋), 게인 세팅, NVM 저장. 부팅 자동 실행은 axis0/axis1 모두 비활성화 (안전) | 안전 헬퍼(`safe_set`, `wait_idle`, `dump_errors`) 포함. **HALL 트랙 모든 작업 전 1회 필수** |
