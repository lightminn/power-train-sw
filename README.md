...
│   ├── python/                      # CPU 포팅 (NumPy / SciPy) + 최종 결과 pkl(v4 f_opt 0.2004)
│   ├── python_gpu/                  # GPU 포팅 (JAX / CUDA 12.x) — v3
│   ├── python_gpu_triangle/         # v4 권위본 (15차원·7지형 + 검증/시각화 도구)
│   ├── archive/                     # 초기 v4 결과(f_opt 0.2624) 이력 보존
│   ├── zetin_v4_viewer_opt.html     # v4 결과 뷰어
...
│   │   ├── gst_stream.py               # 공용 송신 파이프라인 (H.264 SW + SRT)
│   │   ├── yolo_depth_3d.py            # YOLO26+depth 3D 좌표 (영상 SRT + 좌표 UDP)
│   │   ├── yolo_cuda_stream.py         # Jetson CUDA/TRT USB 카메라 송신
│   │   ├── realsense_test.py           # RealSense D435i depth+color 점검
│   │   ├── realsense_stream.py         # RealSense color+depth 진단 송신 (원격주행용 아님)
...
│   ├── corner_module/               # 코너 모듈 (조향+구동 협조 제어 + US-100 게이팅 텔레옵)
...
│   ├── Dockerfile.jetson            # Jetson (dustynv/l4t-pytorch:r36.4.0 + python-can + RealSense SDK)
│   ├── docker-compose.yml           # x86 기본 (CPU/OpenVINO — 개발·테스트용)
│   └── docker-compose.jetson.yml    # Jetson (runtime: nvidia, /dev/ttyTHS1)
├── scripts/                         # 호스트 측 헬퍼
│   ├── recv_stream.sh               # 노트북: 저지연 네이티브 SRT 뷰어 (오버레이 없음)
│   ├── recv_yolo3d.py               # 노트북: SRT 영상 + 좌표 오버레이 뷰어
│   └── can_setup.sh                 # Jetson can0 1 Mbps 셋업 (mttcan + devmem)
└── docs/                            # 트랙별 설계·계획 문서 + 대회 규정/FSM
    ├── specs/                       # 설계 (요구사항/인터페이스)
    ├── plans/                       # 구현 계획 + 검증 기록
    └── reports/                     # 진행 보고·결과 로그
```
...
**현재 권위본은 v4** (면-기준 물리 수정본, `python_gpu_triangle/`): **15차원 · 7종 지형**(계단 / 나무 블록 / 거친 노면 / 단차 / 곡면 경사 / 15° / 30° 경사), 삼각형·프레임 모드 동시 탐색, f_opt 0.2004 (최종 결과 pkl 은 `python/zetin_optimal_params_v4.pkl`). 초기 v4본(f_opt 0.2624)은 `archive/` 에 이력 보존. v3(`python_gpu/`)는 14차원 · 4종 지형.
...
| 파일 | 역할 |
| --- | --- |
| `vision/gst_stream.py` | 공용 송신 파이프라인 빌더 — H.264 SW 인코딩(x264/openh264, **Orin Nano 는 NVENC 하드웨어 없음**) + MPEG-TS + SRT listener. `--srt-latency` 기본 60ms |
| `vision/yolo_depth_3d.py` | **메인** — RealSense D435i + YOLO26 + depth → 검출 물체 3D 좌표(카메라 기준 X,Y,Z·거리·방위/고도각). color 영상은 SRT, 좌표는 UDP JSON 으로 **분리 송신**(영상에 굽지 않아 화질↑·좌표 재사용 가능). 영상을 추론 전에 먼저 보내 지연 최소화. 정렬(rs.align) 대신 검출별 color→depth 투영. 기본 848×480(16:9)·TRT. 수신은 `recv_yolo3d.py`(오버레이) 또는 `recv_stream.sh`(저지연) |
| `vision/yolo_cuda_stream.py` | YOLO26 (PyTorch CUDA / TensorRT FP16) + GStreamer H.264/SRT 송신 (USB 카메라). Jetson 컨테이너 안에서 실행 |
| `vision/realsense_test.py` | Intel RealSense D435i 점검 — depth+color 프레임 수신·중앙거리·유효픽셀 출력 (Jetson 컨테이너) |
| `vision/realsense_stream.py` | RealSense color+depth → GStreamer H.264/SRT 진단 송신. `--mode sidebyside`(color\|depth) / `overlay`. depth 컬러맵은 압축 비효율이라 **원격주행용 아님**(진단 전용) |
...
**노트북 수신 2경로** (둘 다 SRT caller, IPv4 강제):

| 수신 스크립트 | 용도 |
| --- | --- |
| `scripts/recv_stream.sh [PORT] [HOST] [LATENCY]` | **저지연 네이티브 gst 뷰어** (오버레이 없음) — 원격주행용 |
| `scripts/recv_yolo3d.py` | **좌표 박스 오버레이** cv2 뷰어 (`--scale`/`--clock`) — 정밀 접근·좌표 점검용 (표시 지연 약간 더 큼) |
...
### 안전: US-100 충돌방지 (`safety_us100/`)

US-100 거리값을 **안전(safe) / 주의(warn) / 멈춤(stop)** 으로 판정만 내보내는(직접 모터 제어 안 함, publish-only) 모듈. 못 읽으면(연속 실패) fail-safe 로 `stop`, 위험→안전 복귀 시 히스테리시스로 깜빡임 방지.

| 파일 | 역할 |
| --- | --- |
| `safety_us100/evaluator.py` · `safety_monitor.py` · `verdict.py` · `config.py` | 거리→판정 로직·감시 본체·결과형·기준값(stop 200 / warn 400 / hyst 30 mm) |
| `safety_us100/us100.py` | 실 센서 UART 읽기 (`/dev/ttyTHS1`) |
| `safety_us100/fake_sensor.py` · `tests/` | 무하드웨어 테스트 더블 + 자동 시험 |
| `safety_us100/demo.py` | 실 센서로 거리·판정 확인 |

코너 모듈 텔레옵이 이 모듈을 물려 `stop` 판정 시 구동을 0으로 막는다. 상세: `motor_control/safety_us100/README.md`.
...
| `corner_module/teleop_dualsense.py` | DualSense 텔레옵 — `python3 -m corner_module.teleop_dualsense`. US-100 충돌방지(`safety_us100`) 연동 — `stop` 판정 시 구동 0 |
...
| 파일 | 대상 |
| --- | --- |
| `docker/Dockerfile` | x86_64 개발 이미지 (Ubuntu 22.04 + Python 3.10 + ODrive · pygame · OpenCV · ultralytics · OpenVINO, **CPU 전용 torch** — 약 3.3GB) |
| `docker/Dockerfile.jetson` | Jetson Orin Nano 배포 (`dustynv/l4t-pytorch:r36.4.0` 기반, OpenVINO 제외, RealSense SDK(librealsense+pyrealsense2) 소스 빌드 포함) |
...
> x86 이미지는 **CPU 전용**(코드 작성·테스트·OpenVINO). **YOLO GPU 추론은 Jetson** 에서만 한다 — 아래 Jetson 항목 참고.
...
YOLO26 + depth 3D 좌표 + 영상 스트리밍 동작 확인:

```bash
# (Jetson 컨테이너 안) 검출+3D좌표 송신 — 영상 SRT + 좌표 UDP
python3 motor_control/vision/yolo_depth_3d.py --backend trt --host <노트북IP>

# (노트북, 호스트 OS) — 둘 중 하나로 수신
scripts/recv_stream.sh                       # ① 저지연 (오버레이 없음) — 원격주행용
python3 scripts/recv_yolo3d.py               # ② 좌표 박스 오버레이 — 정밀 접근·점검용
```

RealSense D435i (RGB-D) 점검·진단 스트리밍:

```bash
# (Jetson 컨테이너 안) depth+color 점검
python3 motor_control/vision/realsense_test.py

# (Jetson 컨테이너 안) color+depth 진단 송신 (sidebyside / overlay — 원격주행용 아님)
python3 motor_control/vision/realsense_stream.py [--mode overlay]
...
> 부분은 그대로 동작.
> **Jetson Orin Nano 는 NVENC 하드웨어 인코더가 아예 없다**(Orin NX/AGX 만 탑재) —
> `nvv4l2h264enc` 는 어떤 조합으로도 동작 불가, GPU 인코딩 대안도 없음. 따라서
> 소프트웨어 인코딩이 유일한 경로: 기본 `x264enc`(zerolatency, plugins-ugly 필요),
> 폴백 `openh264enc`. 848×480/30fps 까지 ARM A78AE 6코어로 충분.
> 전송은 SRT(ARQ 손실복구) — 혼잡 WiFi 대비. `--srt-latency` 로 지연↔안정성 조절.
