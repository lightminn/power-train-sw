...
│   ├── python_gpu_triangle/         # GPU 변형 — triangle 모드 (v4 초기본)
│   ├── new_parameter_calc/          # v4 면-기준 물리 수정본 (현재 권위본, f_opt 0.2004)
...
**현재 권위본은 v4** (면-기준 물리 수정본, `new_parameter_calc/` 안의 `python_gpu_triangle/`): **15차원 · 7종 지형**(계단 / 나무 블록 / 거친 노면 / 단차 / 곡면 경사 / 15° / 30° 경사), 삼각형·프레임 모드 동시 탐색, f_opt 0.2004. 최상위 `python_gpu_triangle/` 는 v4 초기본(참조용 보존). v3(`python_gpu/`)는 14차원 · 4종 지형.
