...
> 실행일: 2026-07-11 KST  
> 상태: **NEEDS_CONTEXT — 연결 상태 단독 HIL 완료, 물리 분리·재연결 미실행**  
...
L515 unplug은 수행하지 않았다. 따라서 다음은 미검증이다.

1. 분리 시 네 L515 data topic 중단
2. D435i fallback 부재
3. 같은 L515 재연결 뒤 자동 복구

다음 단계는 사용자가 물리 분리를 승인하고 현장에 있을 때만 진행한다. 승인 후 작업자가
명시적으로 요청하는 순간 L515 USB만 뽑고, 검증 뒤 같은 포트에 다시 연결해야 한다. firmware
update나 물리 reset은 수행하지 않는다.
...
`task-6-topics-after.log`를 보존했다.
