...
> **상태: HIL COMPLETE — 기존 10모터 실증과 2026-07-11 안전 HIL 종합 판정**
>
...
> 수행 당시 ODrive node 13·14는 일시 부재했다. present ODrive 11·12·15·16은 CAN
> 캘리브레이션 4/4 성공했고 AK 1~4와 함께 heartbeat가 확인됐다. 13·14는 이전 개별 동작과
> 10모터 통합 HIL에서 정상 확인됐으므로, 두 실행 이력을 합쳐 WP5.1 HIL **완료**로 판정한다.
> 지상 제동과 최종 `stop_mm` 선정은 차체 조립 후 실차 커미셔닝으로 이관한다.
...
- fail-safe: 일시 부재한 13·14를 stale로 검출해 전체 섀시가 latched `ESTOP`; CAN error/passive/bus-off delta 0.
...
### 1.3 후속 실차 커미셔닝 — 시나리오 9, 50 kg 지상주행
...
| 10모터 통합 | `PASS (기존 HIL 증거 승계)` | node 11~16 포함 기존 `/cmd_vel → 10모터` 실증; 13·14는 이번 실행에만 일시 부재 |
| 지상 제동·최종 `stop_mm` | `DEFERRED COMMISSIONING` | 차체 조립 후 실차 질량·노면 조건에서 측정; WP5.1 완료 게이트 아님 |
...
| ODrive 13·14 일시 부재 | 전체 motor stale을 latched E-stop으로 처리 | 재장착 시 연결·캘리브레이션 확인; 기존 정상 이력 유지 |

### 7.3 최종 판정

| 항목 | 기록 |
|---|---|
| 최종 go/no-go | **GO — WP5.1 HIL COMPLETE** |
| 판정자 / 시각 | Codex+사용자 / 2026-07-11 KST |
| 미통과 항목 | 없음. `INVALID_READING` 실측은 환경상 생략하고 자동시험으로 보완 |
| 후속 커미셔닝 | 차체 조립 후 ODrive 13·14 재장착 확인, 지상 제동거리와 최종 `stop_mm` 튜닝 |
| 실행 commit / 설정 | `ec452f6474b6fc57437d576298f2bc954649be42`; can0 500 kbps; HIL 후보 stop_mm 200 mm |

다음 개발은 command-authority spec이며, 이후 L515 경량 color/depth image+IMU pipeline
