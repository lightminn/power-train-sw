...
> 워크스페이스 3패키지 build와 `powertrain_ros` 23 tests passed. 배포 commit
...
|---|---|---|---|
| 지원 `motor_control` suite | **189 passed** | 없음 | commit `49831bb42058a177ed9c41d72d0273f4f0a8f535`; tool capture, raw log 미보존 |
| `motor_gui/tests` | **91 passed** | 없음 | commit `49831bb42058a177ed9c41d72d0273f4f0a8f535`; tool capture, raw log 미보존 |
...
| provenance | commit `49831bb42058a177ed9c41d72d0273f4f0a8f535`; tool capture, raw log 미보존 |
...
`powertrain_canwatchdog`는 반드시 `true`여야 한다. 경쟁 chassis/teleop가 나오면 소유자를
확인해 해당 PID만 정상 종료하고 같은 조회가 무출력임을 확인한다.
...
로컬 189/91과 임시 ROS 3패키지/23 tests는 pre-HIL commit
`49831bb42058a177ed9c41d72d0273f4f0a8f535`에서 tool capture했지만 raw log를 보존하지
않았다. PASS 수치는 유지하되 최종 판정용 원시 증거로 확대하지 않는다.
...
현재 존재하는 원시 결과는 Jetson pytest XML
`/home/zetin/power-train-sw/ros2/build/powertrain_ros/pytest.xml`뿐이다. FAKE summary와 로컬
189/91/ROS23 raw log는 보존되지 않았다. 최종 재실행 때 아래 경로를 생성해 commit·명령·stdout,
JUnit/XML, CAN counter, rosbag과 시나리오 기록을 영속 보존한다. 아래는 **계획 경로**이며 아직
존재한다고 주장하지 않는다.
