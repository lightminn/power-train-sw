...
후속 `60a813f555d5d85fd58bed769853ee5995974f80`는 ROS test 파일만 변경했으므로 위 production
...
| provenance | commit `60a813f555d5d85fd58bed769853ee5995974f80`; `.superpowers/sdd/final-ros-60a813f.xml` |
...
| 격리 read-only ROS 3패키지 clean build | `PASS` | commit `60a813f555d5d85fd58bed769853ee5995974f80`; `robot_arm_msgs`, `powertrain_msgs`, `powertrain_ros` |
...
로컬 189/91 JUnit은 e163eed, 격리 ROS 3패키지 clean build와 30/30 JUnit은
`60a813f555d5d85fd58bed769853ee5995974f80` 증거다. 이 commit은 ROS test 파일만 변경해
production Python 범위는 e163eed와 동일하다. 이후 test-only commit
`839103dc2ae7d8abe0e2c789849bae58eb1e9287`에서 agent가 관찰한 31-test 결과는 추가
참고자료이며 root-observed `.superpowers/sdd/final-ros-60a813f.xml`의 30/30 결과를 대체하지 않는다.
