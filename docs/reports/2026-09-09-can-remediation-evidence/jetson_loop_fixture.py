...
assert '/workspace/ros2/install/' in installed_chassis.__file__, installed_chassis.__file__
   ...
   if now>=next_status:
    ...
    for n in range(1,5):
     bus.send(can.Message(arbitration_id=(41<<8)|n,is_extended_id=True,
                          data=struct.pack('>hhhbb',0,0,0,25,0)),timeout=.05)
    next_status=now+.02
   msg=bus.recv(.002)
   if msg is None or msg.is_extended_id or msg.is_error_frame:continue
