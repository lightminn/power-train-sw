...
> **2026-08-24 갱신 — 실기 주행 확인됨.** 이 경로로 실제 하드웨어가 도는 것을
> 확인했고, 그 근거로 `spec/usb-skid-steer` 22커밋을 main 에 병합했다(`2f602b0`).
>
> 다만 **정량 측정 기록은 아직 남아 있지 않다.** 아래 §4 의 P0(USB 왕복 지연이
> 50 Hz 를 지키는가) · P1(무통전 조향축이 밀리는가)은 수치로 확인·기록된 바 없으므로
> 점검 항목으로 그대로 둔다. 측정하면 이 문서에 실측값을 적을 것.
