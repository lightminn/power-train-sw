"""Read-only reboot acceptance: USB config and passive ten-motor CAN heartbeat."""
import collections
import datetime
import json
import pathlib
...
with motor_session('mks_post_powercycle_readback'):
    ...
    (out / 'post-reboot-result.json').write_text(json.dumps(record, indent=2) + '\n')
    ...
    if '--clear-recovery-latch' in sys.argv:
        for axis in (board.axis0, board.axis1):
            assert axis.current_state == 1 and axis.error in (0, 0x4000)
            axis.clear_errors()
        record['manual_clear_only'] = True
        time.sleep(.1)
