"""Manual Ackermann: installed ROS nodes and CAN drivers on isolated vcan77."""
import json,os,pathlib,signal,subprocess,time,threading,struct
import can
assert os.environ['ROS_DOMAIN_ID']=='77'
assert set(p.name for p in pathlib.Path('/sys/class/net').iterdir()) <= {'lo','vcan77'}, 'network isolation required'
import rclpy
from powertrain_msgs.msg import ManualDriveCommand,SafetyVerdict,WheelStates
...
summary={'virtual_can_actuators':True,'domain':77,'manual_topic':'/teleop/drive_command','max_feedback':0.,'last_mode':None,'last_feedback':None,'wheel_messages':0,'stopped_after_motion':False}
def wheels(message):
...
def received_manual(message, info):
    ...
    summary['last_manual_command']={'speed_mps':message.speed_mps,'steering':message.steering}
node.create_subscription(ManualDriveCommand,'/teleop/drive_command',ReceiptTimeCallback(received_manual),1)
commands=[
 ...
 ['ros2','run','powertrain_ros','chassis','--ros-args','-p','fake:='+('true' if os.environ.get('NEGATIVE_FAKE_ONLY')=='1' else 'false'),'-p','channel:=vcan77','-p','manual_command_format:=steering','-p','authority_enabled:=true','-p','console_estop_latch_path:='+str(base/'latch.json'),'-p','mission_id_path:='+str(base/'mission-id'),'-p','safety_startup_timeout:=5.0'],
 ...
 try:
  while not emulator_done.is_set():
   ...
   if msg.is_remote_frame:
    ...
    requested_velocity=struct.unpack('<ff',msg.data)[0]
    # Preserve the actual wire command even when an IDLE emulated axis does
    # not execute it; otherwise an unsafe nonzero command could look like 0.
    raw_observation['drive'][str(n)]=requested_velocity
    velocities[n]=requested_velocity if states[n]==8 else 0.
