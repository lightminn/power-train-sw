...
| `fd50203`+`e3f2c77` | **WP8 실팔 핸드셰이크 E2E 하네스 + 게이트 1단 통과(07-19)** — 스펙 `2026-07-19-wp8-handshake-e2e-design.md`, 계획 `2026-07-19-wp8-handshake-e2e.md`. 순수 판정기(`wp8_scenario.py`)+rclpy probe(fake-arm 내장·단일송신 가드)+오케스트레이터(`scripts/wp8_handshake_e2e.sh`, 도메인 77). **젯슨 실기: 실팔 `arm_fsm`(headless) 상대 Phase 1 PASS×2**(10 Hz heartbeat, conjunction 수락 `WORK_READY→PERCEIVING`, SIGSTOP 1.5 s 워치독 default-deny 후 재전송 conjunction 재수락) + **Phase 2 PASS×2**(풀사이클 pickup→`CARRYING_LOCKED`→재출발→drop→`STOWED_LOCKED`→재출발, mission_id 단조; fault 4종: no_response 재전송 2회·재출발 금지 / late_done 무시 / FAILED 래치 / dup 완료 7회에도 재출발 1회) + **음성 대조 PASS**(무자극 관측 → probe FAIL). 실기 발견 3건 수정: ①bare 기동 시 `authority_enabled` 없으면 WheelStopPredicate 미생성 → `wheel_stop_unqualified` fail-closed ②MISSION_STOP·arrival 동일 tick 발행 — 크로스토픽 DDS 순서 비보장(24 ms skew 실측) → 판정을 `stop_covers_arrival`(0.5 s 동시활성, 사이 주행모드 금지)로 정정 ③PGID 파일 재사용 → stale 그룹 kill·chassis 누수(수정: 기동 전 rm). **놀람: headless 실팔이 `STOWED_LOCKED`를 발행해 fail-closed 분기가 아닌 풀 경로가 열림.** 게이트(스펙 §5): 1단 통과 — 증적 젯슨 `/tmp/wp8_e2e_20260719T125444Z`(run4)·`...125821Z`(run5)·`...125758Z`(음성). 2단 = 협조 세션 실서보 재실행(하네스 그대로), `contract_v2_verified`는 여전히 기본 false. ⚠️ 젯슨 probe 엔트리는 install/에만 있음(팀원 브랜치 setup.py는 원복) — 재빌드 시 setup.py 한 줄 재추가 필요. 부수 발견: 텔레메트리 유닛 ExecStop pkill 패턴 불일치로 restart마다 구 sender 누수(구 IP 송신 좀비 1개 정리, 유닛 수정은 백로그) |
...
| `65d4ef6` | **콘솔 GUI 전 유즈케이스 E2E 리뷰·수정 20건(07-19 밤)** — 정본 `docs/reports/2026-07-19-console-gui-e2e-review-fixes.md`. 핵심: ①**비상정지 즉시발동 회귀 복원**(3e4050c R06이 승인 스펙 D2 모른 채 2단 확인으로 되돌리고 테스트 고정 — 스펙 인지 없는 리뷰의 교훈) ②브로커 유휴 10초 절단(실측 t=10.0s) 제거 + 콘솔 push-하트비트 2s 링크 사망 감지 ③`mission_clear_grip_lost` Trigger/SetBool 계약 불일치 + 계약↔chassis 타입 정합 게이트 테스트 ④팔 텔레메트리 독성 패킷 스레드 사망(실재현)·합법 JointState가 :5007 전체 침묵 ⑤CAN UNAVAILABLE→LIVE 오표시 ⑥SRT 웻지 5s 워치독 ⑦부재 서비스 3s abandon·estop 재전송 멱등·잠금해제 취소 행·브링업 안내 IP 교정. 검증: dev 1380+3skip / ros x86 659 / 콘솔 165 / smoke PASS + **실 E2E**(유휴 25s 무절단, 콘솔 전 패널 LIVE — 영상 2패널 17.4/24.4 fps 포함, traceback 0). ⚠️ 젯슨엔 수정 5파일이 **install 아티팩트로만** 반영(팀원 브랜치 무접촉, 백업 /tmp/gui_fix/backup) — **재빌드 시 되돌아감, 브랜치 동기화 때 정식 배포 필요**. Codex 2갈래 위임 중 콘솔 갈래 중도 사망 → F7~F10+A#2/A#6은 리뷰어 직접 마무리 |
