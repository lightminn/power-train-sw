...
1. **WP5.3 Task 6**(콘솔 CAN 표시 일원화) → **WP6-S P1 hidden-seed 폐루프**
   (WP6-C 컨트롤러 `c744936` 생겨서 이제 닫을 수 있음).
   (WP6-S P0 2부 `e22e364`, WP6-B 코어 `eba8b74`, WP6-C `c744936` 완료 —
   `powertrain_sim/README.md`와 `powertrain_autonomy/README.md`가 각각 시뮬레이션·
   terrain+controller 계약 정본. WP6-B/C 잔여 게이트 = 장착각 HIL·JAX 자격화·
   프로파일 잠정값 HIL·실기 terrain 유도 스모크.)
2. **D 런북 — WP5.2 원격 E2E 실기 스모크**: authority_enabled 경로 launch 플래그,
   wheel-stop 자격화 완료로 핸드오버 시험 가능해짐. FULL HIL 벤치 항목.
3. 기술 백로그: ①ARMED 유휴 ~530 ms 주기 스톨 — 07-17 격리 재현 시도 결과
   **chassis_node 단독은 결백**(ROS_DOMAIN_ID=42 일회용 컨테이너 FAKE 소크 30분 =
   90,000 tick, >100 ms 갭 0건; 로그 젯슨 `~/wp53_soak/`). 재현에는 실 HIL 스택
   (실 CAN·US-100·동거 컨테이너·도메인 0) 필요 — 다음 실기 벤치에서 동일 로거로
   라이브 /wheel_states 관측할 것. ②health matrix 유휴 플래핑 — **근본원인 확정**:
   `CornerModule.tick()`이 비ARMED 즉시 return이라 유휴 시 CAN 수신 drain 부재 →
   캐시 나이 증가. **수정 완료 `3c1e098`** — 비ARMED tick이 반응 없는 RX 서비스
   (`_service_receive`)를 수행. 실기 유휴 벤치에서 플래핑 소멸 확인만 남음.
