...
| **WP6-C (autonomy controller + command authority)** | 🔜 **다음 개발 항목** | WP6-A/B 산출물 소비, `/autonomy/cmd_vel` 발행 |
...
1. **WP6-C — autonomy controller + command authority**(마스터 계획 §9 순서 12;
   WP6-A 상태추정 + WP6-B terrain 출력 소비, `/autonomy/cmd_vel` 발행). 완료 후
   WP5.3 Task 6(콘솔 CAN 표시 일원화 포함) → P1 hidden-seed 폐루프.
   (WP6-S P0 2부 `e22e364`, WP6-B 코어 `eba8b74` 완료 — `powertrain_sim/README.md`와
   `powertrain_autonomy/README.md`가 각각 시뮬레이션·terrain 계약 정본. WP6-B 잔여
   게이트 = 장착각 HIL·JAX 자격화.)
