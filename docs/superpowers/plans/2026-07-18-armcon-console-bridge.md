...
- 커밋 체인: `93e26db`(r2) → `3ed5610`(T1 브리지) → `d27e537`(T2+T3 콘솔) →
  `7cdb7f0`(문서). 전 커밋 main push·젯슨 동기.
- 3환경: 호스트 332 / dev 1163+2skip / ros 511 (기존 대비 +16/+17/+26, 실패 0).
  크로스 계약 테스트가 :5003·:5007 페이로드를 실제 콘솔 파서로 왕복 봉인.
- 젯슨 실기: parity 511×2 + **설치 엔트리포인트 스모크 PASS**(`ros2 run powertrain_ros
  arm_console_bridge`, 도메인 77·ephemeral 포트·fixture 4토픽 — 2모터 각도/온도,
  조인트, yaw 0.6, is_pick_target True, source_age_s 실측 왕복; killpg 좀비 0).
- 리뷰어 보정 2건: 비유한 yaw 검출 skip(datagram 전체 드롭 방지), Codex의 콘솔측
  강화(엄격 schema 타입 검사·oversize 감지·온도 이벤트 dedup) 검토 수용.
- 이월(협조 세션): 팔 스택 라이브 E2E(perception 가동 시), 온도 임계 실값,
  current 스케일, :5003 단일송신 조율. ⚠️ `/joint_states` 이중발행 감시(현재 팔
  broadcaster 단독 — 우리 스택 발행 시 브리지 이름 필터 추가).
