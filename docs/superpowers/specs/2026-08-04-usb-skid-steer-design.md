...
`[0, 1, xᵢ]·(vx, vy, ω) = drive·sin(0°) = 0` 을 넣는다. 앞뒤 `x` 부호가 반대라
이 식들은 서로 `vy = 0`, `ω = 0` 을 요구하는데, **종방향 6개 식은 `ω = 명령값`
을 요구**하므로 가중최소자승이 그 사이에서 타협한다.

**2026-08-05 실측** (제자리선회 ω 명령 = 0.8 rad/s, `skid_geometry()` 로 만든
관측을 각 기하로 되푼 값):

| 기하 | vy | ω | 배제 |
|---|---|---|---|
| `skid_geometry()` (정답) | 0.00000 | **+0.80000** | 없음 (used 6) |
| `default_geometry()` (기본 설정) | **−0.35016** | +0.80000 | **뒷바퀴 2개 오배제** (used 4) |
| `default_geometry()` + `max_reject=0` | −0.11407 | **+0.39093 (49 %)** | — |
| `four_wheel_geometry()` | −0.09214 | **+0.31577 (39 %)** | — |

⚠️ **이 표가 이전 서술을 대체한다.** 초판은 "ω ≈ 0 으로 눌린다"고 적었으나
측면식을 경성 제약으로 오독한 것이다. 실제 증상은 구성마다 다르다.

- **6륜 기본 설정**: ω 는 우연히 살아난다 — 이상치 배제가 뒷바퀴 2개를
  "슬립"으로 버리면서 모순되는 측면식이 함께 빠지기 때문이다. 대신 **유령
  횡속도 −0.35 m/s** 가 생기고 **정상 바퀴 2개가 오배제**되어 슬립 감지와
  잔차 신뢰도 지표가 무력화된다.
- **4륜 구성 · 배제 여유가 없는 경우**: ω 가 **39~49 % 로 과소추정**된다.
  이쪽이 더 위험하다.

어느 쪽이든 추정이 오염되므로 기하 동기화는 필요하다.
