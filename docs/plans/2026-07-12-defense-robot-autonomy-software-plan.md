...
|---|---:|---|---|
| 1. 스모그 정찰 | 스모그 35, 피아식별 20, 완주 15 | local path, wheel+IMU, 로봇팔 인식 연동 | RGB/depth 동시 저하, 트랙 이탈 |
...
| 4. 겨울 | 빙판 35, 제설 25, 완주 10 | 슬립 감지, 저가감속, 구간 상태머신 | 제자리 헛돌기, 과도한 조향 |
...
```text
L515 RGB/depth ──> local_path_estimator ─┐
...
### WP6-B. Local path estimator

기존 `lane_follower` 개념을 차선이 없는 펌프트랙에도 적용 가능한 local path로 바꾼다.

- RGB에서 바닥/경계 후보와 원근 기반 진행 방향 계산.
- depth에서 통과 가능 공간, 좌우 여유, 근거리 장애물 계산.
- RGB와 depth 신뢰도를 독립 점수화하고 가용 센서만 사용.
- 출력은 경로 중심 offset, heading error, clearance, confidence, stamp.
- RGB/depth 정합이 필요한 계산은 저주기 소비 계층에서 수행하고 RGB 30 fps 전송 경로를 막지 않음.
- PointCloud2를 ROS에 추가하지 않고 depth image를 직접 소비.

완료 기준:

- 녹화 영상 회귀시험에서 경계·중심선 결과를 재현.
- 직선·곡선·부분 가림·낮은 대비에서 confidence가 기대 방향으로 변화.
- 입력이 stale하거나 confidence가 하한 미만이면 속도 명령을 만들지 않음.

### WP6-C. Autonomy controller와 command authority

- local path offset/heading을 4WS 속도·yaw-rate 명령으로 변환.
- 곡률, clearance, confidence, tilt, slip에 따라 속도 제한.
- 자율, 원격, mission hold의 우선순위를 하나의 상태머신으로 집행.
- 모드 전환은 먼저 0속도를 확인한 뒤 새 작성자에게 권한을 넘김.
- 자율 프로세스 사망, stale path, stale odom은 자동복구 motion hold.
...
- local path: 녹화 frame fixture, confidence, clearance, stale.
...
- `powertrain_ros`, L515 Gateway, 로봇팔 인식 동시 부하에서 CPU/RSS와 각 rate 측정.
- RGB 30 fps 전송 목표를 우선 보존하고 depth/local path는 주기를 낮출 수 있음.
...
2. 평탄 저속에서 odometry와 local path.
3. 모의 표적 추종과 가림막 재획득.
4. 로봇팔 mock을 이용한 5개 구간 상태머신.
5. 실제 로봇팔과 `MISSION_STOP → ARRIVED_* → DONE → resume` 1사이클.
6. 스모그·모래·자갈·수중·빙판 조건별 degradation 시험.
7. 5개 구간 자율/원격 반복 리허설과 점수표 기록.
...
- WP6-A wheel+IMU 완료.
- WP6-B local path 녹화 재생 baseline.
...
기능 완료는 코드 merge가 아니라 다음 네 조건을 모두 만족해야 한다.

1. 순수 코어 자동시험과 기존 회귀시험 통과.
2. Jetson exact-HEAD 배포와 전체 프로세스 동시 실행.
3. 해당 구간의 정상·센서 단절·프로세스 사망 HIL.
4. 운영자가 로그 없이도 TUI 상태와 절차로 복구 가능.
...
- ROS PointCloud2 추가와 고주기 RGB-depth alignment.
