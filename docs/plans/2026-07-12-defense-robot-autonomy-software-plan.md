...
- `STOW_REQUEST`: 작업 완료·실패·중단 뒤 안전한 주행 자세로 접고 잠그라는 요청.
...
torque hold를 동시에 확인한다. `CARRYING_LOCKED`는 여기에 qualified grip predicate를 추가한다.
하나라도 불충족이면 `STOWING` 또는 실패 상태를 발행하고 locked heartbeat를 만들지 않는다.
...
`DONE`으로 해석하지 않는다. 활성 미션이 없는 빈 적재 상태의 `STOWED_LOCKED`는 `mission_id=0`을
사용한다.
...
- 독립 채널 진단은 ROS/DDS heartbeat와 RTT, SRT submit/sent/drop, remote input freshness,
  command owner, arm heartbeat, CAN telemetry, L515 Gateway 상태를 한 health snapshot에 합치되
  하나의 장애를 다른 채널 장애로 뭉뚱그리지 않는다.
...
- ROS, SRT, remote, arm, CAN, L515 프로세스를 하나씩 kill/restart해 해당 채널만 정확히 열화되고
  고아 프로세스·중복 owner 없이 복구됨.
...
- 영상 profile은 운용 중 새 encoder graph를 조립하지 않고 사전 HIL한 정적 구성을 전환한다.
  `NORMAL`은 RGB 1280×720 30 fps와 선택된 depth/overlay를 제공한다. `CONGESTED`는 해상도와
  30 fps를 먼저 보존하면서 bitrate를 단계적으로 낮춘다. `EMERGENCY_REMOTE`는 RGB 30 fps를
  최우선으로 남기고 overlay/depth SRT를 끈다. 진입·복귀에는 RTT·loss·send/drop 지표와 hysteresis,
  최소 유지시간을 사용해 profile 플래핑을 막는다.
- sender의 submit/sent/drop은 열화 힌트일 뿐 수신 성공의 권위가 아니다. receiver는 decode/display fps,
  frame age, sequence gap, RTT/loss heartbeat를 역방향 control channel로 보고한다. 이 feedback가 stale하면
  실제 화면을 확인할 수 없으므로 `REMOTE_VIDEO_UNAVAILABLE`로 처리한다.
- profile 전환 acceptance는 전환 순간 fps 하나로 판정하지 않는다. orphan/encoder overlap 0,
  최대 blackout, 첫 IDR 수신시간, 전환 뒤 첫 완전한 5초 window의 receiver ≥29 fps를 기록한다.
  blackout 동안 command는 미리 정한 hold 또는 최저속 정책을 따르며 마지막 영상에 의존해 계속 달리지 않는다.
- 최저 qualified bitrate에서도 RGB receiver 29 fps를 유지하지 못하면 해상도·fps를 몰래 낮추지
  않고 `REMOTE_VIDEO_UNAVAILABLE` motion hold로 전이한다. 영상 없는 원격주행은 허용하지 않는다.
...
- ROS/DDS, SRT, remote input, arm heartbeat, CAN telemetry, L515 Gateway를 하나씩 kill/restart하고
...
15. 네트워크 profile별 SRT 열화·복구와 profile hysteresis 시험.
...
4. Task 4 원격 입력 공통 authority와 qualified wheel-stop predicate.
...
13. WP5.3 Task 6 remote profile/assist, Task 7 regression, Task 8 최종 HIL.
...
- 네트워크 profile과 반자동 원격 보조가 RGB 30 fps와 공통 chassis safety gate를 우회하지 않음.
