...
- 시작점은 하드웨어도 ROS도 필요 없는 **②(순수 수학) — 오늘 노트북에서 바로 시작 가능.**
...
| **python-can** | CAN 버스 송수신 라이브러리 | 8모터 제어 (이미 검증된 프레임 그대로) |
...
**재사용하는 기존 자산** (새로 안 만듦): `corner_module`(조향+구동 협조 제어, HIL 검증) · 8모터 CAN 버스(6/29 전수 검증) ·
...
```
 ...
 /chassis_mode  ◀───┼──┐     │                                        │
                    ...
                    ChassisManager (4코너 통합, 신규)
                       ├─ kinematics.py (4WS 계산기, 신규)
                       └─ CornerModule ×4 (기존)
                            ├─ SteerAk40 (기존, CAN)
                            └─ DriveOdriveCan (스텁 → WP1에서 구현)
                                     ▼
                        can0 (500k) → AK ×4 + ODrive ×4
```
...
### WP1. CAN 구동 드라이버 완성 — `DriveOdriveCan` ★첫 하드웨어 작업

- **무엇**: `motor_control/corner_module/drive_odrive_can.py`가 현재 **껍데기(스텁)**. 코너모듈이 구동을 USB로만
  할 수 있어서 4바퀴 동시 제어가 불가능 → CAN 버전을 채워 넣는다.
