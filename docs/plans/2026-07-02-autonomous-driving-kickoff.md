...
- **진행 상황: WP2·WP3 완료(위 배너). 다음 1순위 = WP1(CAN 구동 드라이버) → WP3 실기 4WS(HIL)의 유일한 관문.**
 ...
 /chassis_mode  ◀───┼──┐     │                                        │
                    ...
                    ChassisManager (6코너 통합, ✅완료 efc0b59)
                       ...
                       └─ CornerModule ×6 (조향4 + 고정2, 기존)
                            ├─ SteerAk40 (기존, CAN)
                            └─ DriveOdriveCan (스텁 → WP1에서 구현) ★남은 관문
                                     ▼
...
- **⭐ 현재 최우선(2026-07-03)**: WP3(ChassisManager)는 코드·pytest(53) 완료됐고 **실기 4WS(HIL)의 유일한 관문이 이 스텁**이라 WP1이 다음 1순위. 드라이버 코드는 하드웨어 없이 지금 작성 가능(CAN 프레임 이미 검증), 회전 테스트만 벤치 모터 필요.
- **HIL 필요 하드웨어**: 젯슨 + CAN 트랜시버(ADM3053 — **외부 5V**·종단 60Ω·TX/RX 스왑주의) + 구동 ODrive+BL70200(최소 1노드, 검증셋 node 11·12·15·16) + **전류제한 48V PSU** + **브레이크 저항**(≈2Ω, regen 소산). 4WS 전체면 +AK×4·조향전원 (+원하면 중간2구동 node13/14 세팅·US-100). ⚠️ **안전 필수**: 물리 E-stop(48V 차단)·**바퀴 지면에서 띄우기**·전류제한 저속시작, **단일노드→4노드→6노드** 순서(한 번에 다 X).
