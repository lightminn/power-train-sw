...
이 경로는 **CAN 4WS용 통합 기동·연결 경로**다. 젯슨에서는 `scripts/robot-start`,
노트북에서는 앱 메뉴의 **Powertrain Integrated Console**을 실행한다. 어느 쪽을
...
8/10 상태이며 **실물 CAN 안정성 인수는 미통과**다. 나머지 수신 노드와 Jetson 버스
오류는 0이다. 원인은 미확정이며 실물 비영점 주행·제동 및 USB/NVM 검증도 별도다.
...
1. 젯슨과 로봇 전원을 켜고, 젯슨 호스트에서 아래 한 명령을 실행한다.
...
```bash
# 젯슨 호스트 — 매일 (최초 준비한 저장소에서 실행)
~/power-train-sw/scripts/robot-start
```

2026-09-09 현장 배포는 기존 팀원 체크아웃을 보존하기 위해 별도
`~/power-train-sw-integrated`에 설치했다. 해당 배포의 매일 실행 명령은
...
의도적으로 US-100을 분리한 현장 시험은 **관리자 조작 → US-100 안전 → 확인**으로
...
| 복구·ESTOP·운용 설정 | 기존 역할 토큰과 ops 게이트 :9001, ESTOP 초기화 별도 |
